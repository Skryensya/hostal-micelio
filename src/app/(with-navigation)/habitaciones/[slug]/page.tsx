        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="order-1 md:order-2">
            <div className="flex flex-col gap-4">
              ...
              {/* Resumen de reserva */}
              <div className="bg-surface-2 rounded-standar p-6 border border-border-light">
                <h2 className="text-xl font-semibold mb-4">Resumen de reserva</h2>
                <div className="space-y-4">
                  {/* Fechas */}
                  <div>
                    <h3 className="text-sm font-medium text-text-muted mb-2">Fechas</h3>
                    <div className="flex items-center gap-2">
                      <Calendar className="h-5 w-5" />
                      <span className="text-sm">
                        {dateRange?.from ? (
                          <>
                            {formatDate(dateRange.from, "d 'de' MMMM", { locale: es })}
                            {dateRange.to && (
                              <> - {formatDate(dateRange.to, "d 'de' MMMM", { locale: es })}</>
                            )}
                          </>
                        ) : (
                  ...
                  {/* Formato */}
                  <div>
                    <h3 className="text-sm font-medium text-text-muted mb-2">Formato</h3>
                    <div className="flex items-center gap-2">
                      <Users className="h-5 w-5" />
                      <span className="text-sm">{format?.label || "Selecciona un formato"}</span>
                    </div>
                  </div>

                  {/* Precio */}
                  <div>
                    <h3 className="text-sm font-medium text-text-muted mb-2">Precio</h3>
                    <div className="flex items-center gap-2">
