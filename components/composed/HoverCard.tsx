...
export const HoverCard: React.FC<HoverCardProps> = ({
  ...
  return (
    <Link
      href={link}
      className="aspect-square w-full relative overflow-hidden group rounded-standar cursor-pointer border border-border-light shadow-sm"
    >
      <div className="relative h-full">
        ...
        <div className="absolute inset-0 z-10 p-4 text-white flex flex-col justify-end group-hover:from-10% bg-gradient-to-t from-0% to-40% from-black/60 to-transparent group-hover:to-50% group-focus:from-10% group-focus:to-50% group-hover:from-black/50 group-focus:from-black/40">
          ...
          <p className="text-lg font-medium mt-2 opacity-0 max-h-0 overflow-hidden transition-all duration-500 delay-100 ease-out group-hover:opacity-100 group-hover:max-h-20 group-focus:opacity-100 group-focus:max-h-20">
            {description}
          </p>
        </div>
      </div>
